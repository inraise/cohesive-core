...
// GetSummary godoc
// @Summary Сводка по бюджету дома
// @Description Текущий баланс, суммы приходов/расходов и вклад каждого участника
// @Tags budget
// @Produce json
// @Security ApiKeyAuth
// @Param id path string true "ID дома"
// @Success 200 {object} budget_transport_http.BudgetSummaryDTOResponse "Сводка по бюджету"
...
// @Router /households/{id}/budget [get]
func (h *BudgetHTTPHandler) GetSummary(rw http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	summary, err := h.budgetService.GetSummary(ctx, householdID, callerID)
	if err != nil {
		responseHandler.ErrorResponse(err, "failed to get budget summary")

		return
	}

	contributionsDTO := make([]MemberContributionDTOResponse, 0, len(summary.Contributions))
	for _, c := range summary.Contributions {
		contributionsDTO = append(contributionsDTO, MemberContributionDTOResponse{
			UserID: c.UserID,
			Amount: c.Amount,
		})
	}

	response := BudgetSummaryDTOResponse{
