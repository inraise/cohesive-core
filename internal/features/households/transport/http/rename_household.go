...
func (h *HouseholdsHTTPHandler) RenameHousehold(rw http.ResponseWriter, r *http.Request) {
	...
	var request households_service.RenameHouseholdRequest
	...
	householdWithRole, err := h.householdsService.RenameHousehold(ctx, householdID, callerID, request)
	if err != nil {
		responseHandler.ErrorResponse(err, "failed to rename household")
